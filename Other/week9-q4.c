...
void init_hardware(void) {
    ...
    //Timer Overflow Speed = 8160 micro seconds (Timer Speed * 255) - (255 since 8-bit timer)
    TCCR0B |= (1<<CS02);
    TCCR0B &= ~((1<<CS01));
    TIMSK0 |= (1 << TOIE0);
...
double get_system_time(unsigned int timer_count) {
    ...
    double scalar = 128 / 1000000.0; // It isn't microcontrollers unless you get a good dosage of magic numbers
